	...
	public partial class Win7_new : Form
	{
		...
		private Form CreateForm(WinNumber winNumber)
		{
			switch (winNumber)
			{
				...
				case WinNumber.Dictionary:
					return new Win7_Dictionary();
				default:
					return null;
			}
		}
		...
		private void SetTagsToButtons()
		{
			...
			btnDictionary.Tag = WinNumber.Dictionary;
		}
		...
		{
			//_logger.LogUserAction("Открытие формы Технологические карты");
			await LoadFormInPanel(WinNumber.Dictionary).ConfigureAwait(false);
		}
